        ...
        if (tmp == L"\r") {
            if (gameBoard[row][col] == 0) {
                ...
                if (curGameState.gameMode == GAME_MODE_PVE) {
                    ...
                    std::pair<short, short>moveAI = GetBestMove(gameBoard,
                    moveCount, row, col); GameAction::MakeMove(gameBoard,
                    moveCount, moveAI.first, moveAI.second, curPlayer.value);
                    gameScreen.boardContainer.DrawToBoardContainerCell(row, col,
                    curPlayer.symbol); curGameState.moveList.push_back({
                    moveAI.first, moveAI.second });
                    gameScreen.logContainer.DrawToLogContainer(curGameState.moveList,
                    curGameState.playerNameOne, curGameState.playerNameTwo);*/
                }

                const short GAME_STATE = Logic::GetGameState(
                    gameBoard, moveCount, row, col, curPlayer.value
                );
                switch (GAME_STATE) {
                    case 1:
