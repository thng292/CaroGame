...
namespace Evaluation {
	...
	short GetHorizontalComboEval(
		...
		if (move.col == 0) blockLeft = true;
		else if (board[move.row][move.col - 1] != 0 && board[move.row][move.col - 1] != playerValue) blockLeft = true;
		if (colLast == Constants::BOARD_SIZE - 1) blockRight = true;
		else if (board[move.row][colLast + 1] != 0 && board[move.row][colLast + 1] != playerValue) blockRight = true;
	...
	short GetVerticalComboEval(
		...
		bool blockLeft = false, blockRight = false;
		if (move.row == 0) blockLeft = true;
		else if (board[move.row - 1][move.col] != 0 && board[move.row - 1][move.col] != playerValue) blockLeft = true;
		if (rowLast == Constants::BOARD_SIZE - 1) blockRight = true;
		else if (board[rowLast + 1][move.col] != 0 && board[rowLast + 1][move.col] != playerValue) blockRight = true;
		if (blockLeft && blockRight) evalValue = 0;
		else if (blockLeft || blockRight) evalValue /= 2;
	...
	short GetDiagonalRightComboEval(
		...
		bool blockLeft = false, blockRight = false;
		if (move.row == 0 || move.col == 0) blockLeft = true;
		else if (board[move.row - 1][move.col - 1] != 0 && board[move.row - 1][move.col - 1] != playerValue) blockLeft = true;

		if (rowLast == Constants::BOARD_SIZE - 1 || colLast == Constants::BOARD_SIZE - 1) blockRight = true;
		else if (board[rowLast + 1][colLast + 1] != 0 && board[rowLast + 1][colLast + 1] != playerValue) blockRight = true;
	...
	short GetDiagonalLeftComboEval(
		...
		bool blockLeft = false, blockRight = false;
		if (move.row == 0 || move.col == Constants::BOARD_SIZE - 1) blockLeft = true;
		else if (board[move.row - 1][move.col + 1] != 0 && board[move.row - 1][move.col + 1] != playerValue) blockLeft = true;
	...
	short GetComboEval(
		...
		GameAction::Board comboCheckBoard(Constants::BOARD_SIZE, vector<short>(Constants::BOARD_SIZE, 210));
			...
			for (short col = 0; col < Constants::BOARD_SIZE; ++col) {
				if (board[row][col]) {
					...
					if (comboCheckBoard[row][col] % 2 == 0) {
						short eval = GetHorizontalComboEval(board, comboCheckBoard, { row, col }, board[row][col]);

						evalValue += eval;

					}
					if (comboCheckBoard[row][col] % 3 == 0) {
						short eval = GetVerticalComboEval(board, comboCheckBoard, { row, col }, board[row][col]);
						evalValue += eval;
					}
					if (comboCheckBoard[row][col] % 5 == 0) {
						short eval = GetDiagonalRightComboEval(board, comboCheckBoard, { row, col }, board[row][col]);
						evalValue += eval;
					}
					if (comboCheckBoard[row][col] % 7 == 0) {
						short eval = GetDiagonalLeftComboEval(board, comboCheckBoard, { row, col }, board[row][col]);

						evalValue += eval;
					}


					if (board[row][col] == playerValue) evalResult += evalValue;
