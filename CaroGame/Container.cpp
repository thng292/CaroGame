...
void Container::DrawToLogContainer(
    ...
    std::wstring playerNameOne, std::wstring playerNameTwo
) {
    ...
    for (size_t i = 0; i < numOfValueDisplay; ++i) {
        ...
        std::wstring value = std::format(
            L"Move {}: Player {} ({}) played {}{}", startIndex + 1, playerName,
            playerSymbol, (wchar_t)(L'A' + valueList[startIndex].second),
            valueList[startIndex].first + 1
        );

        FillConsoleOutputCharacter(
            StdOut, L' ', cellWidth - xOffset,
            {xCoord + xOffset, yCoord + yOffset + (short)i}, &tmp
        );
